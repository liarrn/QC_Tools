...
def MCMC_dmol3(xyzfilename, calc_params, mc_params, move_params):
    calculator = 'dmol3'
    temp = mc_params['temp']
    ...
    for i in range(num_steps):
        ...
        writexyzforce(xyz_force_out, 'a', next_elems, next_coor, next_force, '%dth %.9f eV'%(i+2, next_energy))
        ...
        if next_energy<= curr_energy:
            ...
            bf = BoltzmannFactor(next_energy, curr_energy, temp, 'ha')
            rn = np.random.rand()
            if rn < bf:
                is_move = True
        if is_move == True:
...
def MCMC(xyzfilename, calc_params, mc_params, move_params):
    # metropolis MC
    if calc_params['calculator'] == 'dmol3':
        MCMC_dmol3(xyzfilename, calc_params, mc_params, move_params)
...
if __name__ == '__main__':
    ...
    move_params={'method': 'randall', 'step_size': 0.02}
