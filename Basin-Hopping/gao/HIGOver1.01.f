...
! The largest system is 500 atoms in total.
             ...
             write(01,*) 'ENDREAD'
             close(01)
			   open(unit=48,file='CNA',status='unknown')
             close(48)
...
!        implicit double precision (a-h, o-z)
     ...
     *            Avcon = 6.02214199e23)
        ...
        character*2 elem(500)
          ...
          if (PBC_TOGGLE .eq. .FALSE.) then 
		    WRITE (02, '(a)')  'PBC=OFF'
          else if (PBC .eq. .TRUE.) then
		    WRITE (02, '(a)')  'PBC=ON'
...
C                     if(rij.ge.rdmin.and.rij.le.rdmax) then
                    if(rij.lt.rdmin.or.rij.gt.rdmax) then
                      nf=1
                    endif
                  endif
...
100       continue
        ...
        subroutine CNA_CHECK(elem, x, y, z, CNA)
             integer CNA(MXCY, 1000, 1000)
           double precision x(500),y(500),z(500)
           character*2  elem(500)
        end
      ...
      subroutine CNA_CALC(m, elem, x, y, z, CNA)
            integer m
            integer CNA(MXCY, 1000, 1000)
            double precision x(500),y(500),z(500)
            character*2  elem(500)
