...
//used only for spin-polarized calculation
{
	...
	for(i = 0; i < nrbands; i++)
	{
		...
		{
			d_centre = d_centre + (d_spinup + d_spindown) * pdos[i].energy;
			d_dos = d_dos + d_spinup + d_spindown;
		}
	...
	d_centre /= d_dos;
	d_centre -= efermi;
	printf("d_centre with fermi level correction is %f\n", d_centre);
